<?php
	...
	{
         $editId=$_GET['editID'];
		     $edit_kinema=$kinema->getKinemaById($editId);
	...
	<div id="body" class="Adminboard" style="background-color:white;">
		         ...
		         <div class="panel panel-info" style="width:100%;position:relative;margin-bottom:0px;font-size:1.2em;float:left;">
                         <div class="panel-heading">
                           <?php if(isset($_GET['editID'])) echo 'Kinema Details';
						   else echo 'Add Cinema';
						   ?>
                        </div>


							<a href="lista_kinemave.php"><input type="submit" style="float:right;margin-top:5px;"value="Back to Cinema Table"  class="btn btn-info"/></a>
...
<label>Phone Nr:</label>
		    <input type="tel"  name="tel" value="<?php echo $tel; ?>" required placeholder="### ### ####" pattern="06+\d{8}" />
								...
								<thead>
								<tr><th colspan="6">Theatres list</th></tr>
						...
						{
						       		$msg='';
                              $lista_sallave=$kinema->getAllSallas();
		  ...
		  <td><a href="shto_kinema.php?editID='.$editId.'&fshiSalle='.$salla_id.'"><img width="20px" height="20px" src="../images/trashbin.jpg" /><a></td>
      ...
      if(isset($_POST['submit']))
		   {
		    // echo '<script type="text/javascript"> alert("Submit button clicked'.$editId.'")</script>'; // kur shtyp submit del alert qe sapo e ke shtypur butonin
			      ...
			      $tel=$_POST['tel'];
						...
						$kinema->redirect("lista_kinemave.php");
						}
						else
						{
							$kinema->insert($cname,$adresa,$tel);
							echo '<script type="text/javascript"> alert("Kinema is added succesfully !")</script>';
							$kinema->redirect("lista_kinemave.php");
						}
		 ...
		 if(isset($_POST['addtheater']))
		 {
			 ...
			 echo '<script type="text/javascript"> alert("Salla is added succesfully !")</script>';
			$kinema->redirect("shto_kinema.php?editID=".$editId."");
		 }
